...
function getContractsFromRecord(record) {
    ...
    record.contracts.map( (contract, n) => {
        ...
        // computed_contract['classification'] = 'contract';
        ...
        for( var x in record ) {
            switch(x) {
                case 'parties':
                    ...
                    if(supplier_parties.length > 0) {
                        ...
                        supplier_parties.map( (supplier, index) => {
                            ...
                            computed_contract.parties['suppliers']['names'].push(supplier.name);
                ...
                case 'tender':
                    ...
                    if(record.tender.hasOwnProperty('items') && record.tender.items.length > 0 && award.hasOwnProperty('items') && award.items.length > 0) {
                        let tenderItem = record.tender.items.filter( (t_item) => t_item.id == award.items[0].id );
                        tenderObj.items = tenderItem;
                    }
